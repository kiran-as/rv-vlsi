...
$sql = "Select * from tbl_registermoreinfostudents";
	$result = @mysql_query($sql)	or die("Couldn't execute query:<br>".mysql_error().'<br>'.mysql_errno());
	$dbTable="Register more info";
...
//$date=date("d",$imp[p_time])." ".date('M, Y',$imp[p_time])."  ".date('g:i:s',$imp[p_time]);
	...
	str_replace("\t", "t",trim(stripslashes(gmdate("'Y-m-d H:i:s", $imp[updatetime]))))."\t". 
	str_replace("\t", "t",trim(stripslashes($imp[be])))."\t".
	str_replace("\t", "t",trim(stripslashes($imp[me])))."\t".
	str_replace("\t", "t",trim(stripslashes($imp[ind])))."\t".
		str_replace("\t", "t",trim(stripslashes($imp[msg])))."\t";
	$output = preg_replace("/\r\n|\n\r|\n|\r/", ' ', $output);
		print(trim($output))."\t\n";
				}		 
