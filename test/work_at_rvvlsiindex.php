...
$resultsss = "SELECT * from tbl_work_rvvlsi";

	$resultc = mysql_query($resultsss);
	$s=0;
	while ($row = mysql_fetch_assoc($resultc)) {
		  $arraStudent[$s]["name"]	= $row["name"];
		 $arraStudent[$s]["id"]	= $row["id"];
		 	 $arraStudent[$s]["emailAddress"]	= $row["emailAddress"];
		 $arraStudent[$s]["mobileNumber"]	= $row["mobileNumber"];
		  $arraStudent[$s]["domainExpertise"]	= $row["domainExpertise"];

		  $arraStudent[$s]["msg"]	= $row["msg"];
		  $s++;  
		}
?>
...
function deletework(idtestimonials)
{
 var deletess = confirm("Do you really want to delete");
  if(deletess == true)
  {
        
	 window.location = "deletework.php?idfaqdetails="+idtestimonials;
   }
}
</script>
</head>
...
<td><?php echo $arraStudent[$s]['name'];?></td>
<td><?php echo $arraStudent[$s]['domainExpertise'];?></td>
<td><?php echo $arraStudent[$s]['mobileNumber'];?></td>
						  <td>
				<a onclick="deletework(<?php echo $arraStudent[$s]['id']?>)"><img src="../images/icon_delete.png" border="0" alt="Delete" align="right"/></a>
						  </td>
</tr>
